...
class ContainerViewsTestCase(CalipsoTestCase):
    ...
    def test_view_run_stop_rm_container(self):
        ...
        # stop
        response_stop = self.client.get(reverse('stop_container', kwargs={'container_id': container_name}))
        self.assertEqual(response_stop.status_code, status.HTTP_200_OK)

        # remove
        response_rm = self.client.get(reverse('rm_container', kwargs={'container_id': container_name}))
